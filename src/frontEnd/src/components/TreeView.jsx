...
function TreeNode({ node, matchedIds, traversalPath, currentNodeId, viewMode, depth = 0 }) {
  const [expanded, setExpanded] = useState(true); // Selalu expand sepenuhnya secara default
  ...
  return (
    <div className="flex flex-row items-center relative group/node">
      ...
      <motion.div
        ...
        className={`treenode-badge relative flex items-center gap-1 py-0.5 px-1.5 rounded border text-[10px] outline-none transition-all duration-300 cursor-pointer z-10 shadow-sm whitespace-nowrap min-w-max shrink-0 ${
          isVisiting
            ? "bg-yellow-400/20 border-yellow-400 shadow-[0_0_10px_rgba(250,204,21,0.5)] scale-110 z-20" 
            : isMatched
            ? "bg-neonGreen/20 border-neonGreen shadow-[0_0_8px_rgba(16,185,129,0.3)] z-10"
            : isTraversed
      ...
      >
        ...
        {node.elementId && <span className="text-yellow-500 font-bold max-w-[60px] truncate">#{node.elementId}</span>}
        {node.classes?.length > 0 && <span className="text-sky-400 opacity-90 max-w-[70px] truncate">.{node.classes.join(".")}</span>}
      ...
      <AnimatePresence>
        {expanded && hasChildren && (
          <motion.div
            ...
            className="flex flex-col py-0.5 pl-5 relative shrink-0"
          >
            {/* Garis Horizontal Penghubung Utama dari Parent ke Garis Vertikal */}
            <div className="absolute left-0 top-1/2 w-2.5 h-[2px] bg-sky-400/60 -translate-y-1/2 rounded-full" />

            {node.children.map((child, idx) => (
              <div key={child.id} className="relative flex items-center py-0.5 shrink-0">
                {/* Garis Horizontal Menuju Kotak Anak */}
                <div className="absolute -left-2.5 w-2.5 h-[2px] bg-sky-400/60 rounded-full" />
                {node.children.length > 1 && (
                  <>
                    {idx === 0 && <div className="absolute -left-2.5 top-1/2 bottom-0 w-[2px] bg-sky-400/60 rounded-t-full" />}
                    {idx === node.children.length - 1 && <div className="absolute -left-2.5 top-0 bottom-1/2 w-[2px] bg-sky-400/60 rounded-b-full" />}
                    {idx > 0 && idx < node.children.length - 1 && <div className="absolute -left-2.5 top-0 bottom-0 w-[2px] bg-sky-400/60" />}
                  </>
                )}
...
export default function TreeView({ tree, maxDepth, totalNodes, matchedIds, traversalPath, currentNodeId, loading }) {
  ...
  if (loading) return <div className="glass-panel rounded-2xl flex-1 flex flex-col items-center justify-center min-h-[400px] border border-borderDrop/50 text-gray-500 space-y-4">Building DOM Tree...</div>;
  if (!tree) return <div className="glass-panel rounded-2xl flex-1 flex flex-col items-center justify-center min-h-[400px] p-8 border border-borderDrop/50 text-gray-500 text-center">Tree Data Not Loaded</div>;

  return (
    <div className="glass-panel rounded-2xl flex flex-col overflow-hidden h-[90vh] w-full min-w-full">
      <div className="px-6 py-4 border-b border-borderDrop flex flex-wrap items-center justify-between gap-4 bg-surface/30 shrink-0">
        <div className="flex flex-col gap-1">
          <h2 className="text-xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-gray-100 to-gray-400">DOM Tree Diagram</h2>
      ...
      <div 
        ...
        className="p-4 overflow-auto flex-1 relative bg-background/20 select-none cursor-grab active:cursor-grabbing [&::-webkit-scrollbar]:hidden [-ms-overflow-style:'none'] [scrollbar-width:'none']"
      >
        <div 
           className="w-max min-h-fit"
